...
def get_comprehensive_analytics():
    ...
    try:
        ...
        faculty_res = execute_query("SELECT * FROM quality_facultyperformance", fetch_all=True)
        faculty_data = faculty_res or []
        
        accreditation_res = execute_query("SELECT * FROM quality_accreditation ORDER BY report_date DESC LIMIT 1", fetch_all=False)
        latest_accreditation = accreditation_res if accreditation_res else {}
        
        students_res = execute_query("SELECT COUNT(*) as count FROM students", fetch_all=False)
        total_students = students_res.get('count', 0) if students_res else 0
